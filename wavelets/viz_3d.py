...
def main():
    ...
    top_path = os.path.join(exports_dir, 'topology.json')
    
    if not os.path.exists(pred_path) or not os.path.exists(top_path):
        print("Error: Exported data not found. Run main.py first to generate the topology and predictions.")
        return
    ...
    for edge_id, (u, v, L) in enumerate(edge_defs):
        ...
        x_param = data['x'].numpy().flatten()
        z_val = data['u_pred'].numpy().flatten()
        
        if 'u_exact' in data:
            z_ex = data['u_exact'].numpy().flatten()
        else:
            z_ex = None
        ...
        ax.plot(X, Y, z_val, color='red', linewidth=2, linestyle='--', label='Wavelet Approx' if edge_id==0 else "")
        ax.plot(X, Y, 0, color='black', linewidth=1)
        if z_ex is not None:
            ax.plot(X, Y, z_ex, color='blue', linewidth=1, linestyle='-', alpha=0.7, label='True Solution' if edge_id==0 else "")
